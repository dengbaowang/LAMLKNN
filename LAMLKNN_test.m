function [HammingLoss,RankingLoss,OneError,Coverage,Average_Precision,Outputs,Pre_Labels]=MLKNN_test(train_data,train_target,test_data,test_target,Num,centersK,num_cluster,PriorK,Prior,PriorN,Cond,CondN)
...
%Computing distances between testing instances and centers 
    dist_matrix_c=zeros(num_testing,num_cluster);
    for i=1:num_testing
        if(mod(i,100)==0)
            disp(strcat('computing distance between test-point and centers:',num2str(i)));
        end
        vector1=test_data(i,:);
        for j=1:num_cluster
            vector2=centersK(j,:);
            dist_matrix_c(i,j)=sqrt(sum((vector1-vector2).^2));
        end
    end
    %dist_matrix_c;
%Find Cluster for test instance
    WhichK=zeros(1,num_testing);
    for i=1:num_testing
        [temp,index]=sort(dist_matrix_c(i,:));
        WhichK(i)=index(1);
    end
    %WhichK;
%Computing Outputs
...
%         end
        ...
        for j=1:num_class
            Prob_in=PriorK(WhichK(i))*Prior(j,WhichK(i))*Cond(j,temp(1,j)+1,WhichK(i));
            Prob_out=PriorK(WhichK(i))*PriorN(j,WhichK(i))*CondN(j,temp(1,j)+1,WhichK(i));
